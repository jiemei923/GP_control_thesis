...
if isfield(opt,'ndata')
	min_datapoints = opt.ndata;
else
	min_datapoints = 200;
end
...
if isfield(opt,'var_names')
	var_names = opt.var_names;
else
	for n=2:nargin
		var_names{n-1} = inputname(n);
	end
end

if ~isfield(opt,'minval')
	opt.minval = -1e5;
end
if ~isfield(opt,'maxval')
	opt.maxval = 1e5;
end
